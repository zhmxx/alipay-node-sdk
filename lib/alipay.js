...
/**
 * 统一转账接口 https://docs.open.alipay.com/api_28/alipay.fund.trans.uni.transfer/
 * @param {Object} opts
 * @param {String} [opts.outBizNo]      商户唯一订单号
 * @param {String} [opts.transAmount]   转账金额，精确到小数点后两位
 * @param {String} [opts.orderTitle]    转账业务的标题，用于在支付宝用户的账单里显示
 * @param {String} [opts.payeeIdentity] 收款方支付宝账号
 * @param {String} [opts.payeeName]     收款方真实姓名
 * @param {String} [opts.remark]        备注
 */
props.transfer = function (opts) {
    var biz_content = {
        out_biz_no: opts.outBizNo,
        trans_amount: opts.transAmount,
        product_code: 'TRANS_ACCOUNT_NO_PWD',
        payee_info: {
            identity: opts.payeeIdentity,
    ...
    var params = this.makeParams('alipay.fund.trans.uni.transfer', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
